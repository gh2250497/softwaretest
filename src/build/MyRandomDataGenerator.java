 ...
 * @since 3.1
 */
public class MyRandomDataGenerator implements RandomData, Serializable {
     ...
     */
    public MyRandomDataGenerator(RandomGenerator rand) {
        this.rand = rand;
    }
    ...
    public long nextLong(final long lower, final long upper) throws NumberIsTooLargeException {
        ...
        if (max <= 0) {
            ...
            return upper + getRandomGenerator().nextInt((int) max);     //正确为lower
        } else {
            // we can shift the range and generate directly a positive long
            return upper + nextLong(getRandomGenerator(), max);         //正确为lower
        }
    }
    ...
    public int[] nextPermutation(int n, int k)
        ...
        return MathArrays.copyOf(index, n);         //正确为(index,k)
    }
